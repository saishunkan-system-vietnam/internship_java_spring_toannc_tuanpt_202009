<template>
  <div>
    <v-card>
      <v-container>
        <h3>
          <v-icon medium color="green darken-2"> mdi-tennis </v-icon>TOURANMENT
          :{{ tournament.nameTour }}
        </h3>
        ...
        <v-tabs-items v-model="tab">
          <v-tab-item :value="'tab-1'">
            <v-card flat>
              <v-card-text>
                <h5 style="color: rgb(107, 75, 75)">Team</h5>
                <v-simple-table>
                ...
                <h5 style="color: rgb(107, 75, 75)">Rank</h5>
                <v-simple-table>
                  <template v-slot:default>
                    <thead>
                      <tr>
...
export default {
  ...
  methods: {
    detailTeam(id) {
      this.$router.push("/DetailTableTennis/" + id);
    },
    gettournament() {
      this.$store
        ...
        .then((response) => {
          ...
          this.$store
            .dispatch("schedule/getByIdTour", this.tournament.idTour)
            .then((response) => {
    ...
    detail(tournament) {
      ...
      var myWindow = window.open(
       ENV.BASE_NETWORK+"/detail/" + tournament.idSchedule,
        "myWindow",
